...
def login():
    if request.method == 'POST':
        ...
        if not user:
            query_db("INSERT INTO users VALUES (NULL,?,?,?,?,?,?,?)", (u,p,e,role,b_name,b_acc,b_hold), commit=True)
...
def dashboard():
    ...
    jobs = query_db("SELECT * SELECT * FROM jobs")
    if not jobs:
...
def admin_panel():
    ...
    return render_template('admin_pannel.html', all_users=users, job_count=jc['count'], treasury=tr)
