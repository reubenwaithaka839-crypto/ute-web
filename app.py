...
app = Flask(__name__)
app.secret_key = os.environ.get('FLASK_SECRET_KEY', 'supermax_vault_777_final')

# Using a brand new name to force a clean database on Render
DB = "ute_production_final.db"

def query_db(query, args=(), one=False, commit=False):
    conn = sqlite3.connect(DB)
    conn.row_factory = sqlite3.Row
    cur = conn.cursor()
    # Updated Schema with 'status' column
...
def register():
    if request.method == 'POST':
        ...
        # New Admins are set to 'pending_approval'
        ...
        except: return "Registration Error: Username may be taken."
    ...
    if request.method == 'POST':
        ...
        if user and bcrypt.checkpw(p.encode('utf-8'), user['password'].encode('utf-8')):
            if user['status'] == 'pending_approval':
                return "<h1>Access Pending</h1><p>Your Admin account must be authorized by the System Owner (User ID 1).</p>"
...
def admin_room():
    ...
    pending_admins = []
    # Only the very first user (You) can approve other admins
...
def approve_admin(id):
    if session.get('user_id') != 1: return "Access Denied", 403
    # Check if we already have 3 active admins
    active_admins = query_db("SELECT COUNT(*) as count FROM users WHERE role = 'admin' AND status = 'active'", one=True)['count']
    if active_admins >= 3:
        return "<h1>Limit Reached</h1><p>Maximum of 3 active Admins allowed.</p>"
...
def withdraw():
    ...
    if wallet and wallet['balance'] >= gross:
        ...
        query_db("""INSERT INTO transactions (sender, receiver, amount, deduction, net_amount, type, status) 
                    VALUES (?, 'BANK', ?, ?, ?, 'Withdrawal', 'pending')""", 
